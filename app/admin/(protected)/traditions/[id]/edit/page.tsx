...
export default function EditTraditionPage() {
  ...
  useEffect(() => {
    fetch(`/api/admin/traditions/${id}`)
      ...
      .then(data => {
        ...
        setFoodPrinciples(data.food_principles ?? '')
        setCommonIngredients((data.common_ingredients ?? []).join(', '))
        setCookingTechniques((data.cooking_techniques ?? []).join(', '))
        setRepresentativeDishes((data.representative_dishes ?? []).join(', '))
        setPublished(data.published ?? false)
        setLoading(false)
      })
      .catch(() => {
        setError('Failed to load tradition')
        setLoading(false)
      })
  }, [id])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch(`/api/admin/traditions/${id}`, {
      method: 'PUT',
  ...
  if (loading) return <p className="text-charcoal-400 text-sm">Loading…</p>
  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-2xl font-display text-charcoal-100">Edit Tradition</h1>
      </div>
